...
- **`cwd` 위젯 적응형 자동 축약** — `max_length` 미설정 시 터미널 너비의 1/3을 자동 한도로 적용 (#57)
  - 긴 경로가 레이아웃 레벨 truncation으로 잘리는 문제 해결
  - 예: 터미널 120칸 → 최대 40자, 80칸 → 최대 26자
  - 명시적 `max_length` 설정 시 해당 값 우선 적용
  - 자동 축약 테스트 3건 추가
...
- **`full` 프리셋 중복 위젯 제거** — 24 → 22 위젯으로 축소 (#50)
  - `block_timer` 제거: `block_limit`이 남은 시간까지 표시하므로 상위 호환
  - `block_cost` 제거: 세션 비용(`cost`)과 거의 동일한 값
  - Line 7 가독성 개선: 6개 → 4개 위젯
