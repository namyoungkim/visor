...
| 위젯 | 식별자 | 출력 예시 | 설명 |
|-------|--------|----------|------|
| 도구 사용 횟수 | `tools` (확장) | `✓Bash ×7 \| ✓Edit ×4 \| ✓Read ×6` | 도구별 호출 횟수 표시 |
| 에이전트 상세 | `agents` (확장) | `Explore: Analyze widgets (42s)` | 설명 + 소요시간 표시 |

**도구 사용 횟수 구현:**
- 기존: 도구별 마지막 상태만 표시 (`✓Read ✓Write ◐Bash`)
- 변경: 도구 이름별 그룹화 + 카운트 (`✓Bash ×7`)
- 데이터: transcript JSONL에서 tool_use 블록 카운트

**에이전트 상세 구현:**
- 기존: 에이전트 상태만 표시 (`✓Explore ◐Plan`)
- 변경: Task 도구의 description + 소요시간 표시
- 데이터 추출:
  - `description`: Task tool input의 `description` 필드
  - `duration`: tool_use/tool_result 타임스탬프 차이 계산
...
| 기능 | 설명 | 상태 |
|------|------|------|
| TUI 설정 도구 | `visor --tui` 인터랙티브 편집기 | ✅ Charm 생태계 (bubbletea, bubbles, lipgloss) |
| 위젯 관리 | 추가/삭제/순서변경 | ✅ j/k, a, d, J/K 키바인딩 |
| 옵션 편집 | 위젯별 threshold 등 설정 | ✅ e 키로 편집 |
| 실시간 미리보기 | 변경사항 즉시 확인 | ✅ 하단 미리보기 영역 |

### v0.6 — 도구/에이전트 상세 🔲 예정

| 기능 | 설명 | 상태 |
|------|------|------|
| 도구 사용 횟수 | `tools` 위젯에 카운트 추가 | 🔲 `✓Bash ×7 \| ✓Edit ×4` |
| 에이전트 상세 | `agents` 위젯에 설명+시간 추가 | 🔲 `Explore: Analyze widgets (42s)` |

**도구 사용 횟수:**
- Tool 이름별 그룹화 + 카운트 집계
- Extra 옵션: `show_count` (default: true)

**에이전트 상세:**
- Task 도구 input에서 `description` 추출
- tool_use → tool_result 타임스탬프로 소요시간 계산
- Extra 옵션: `show_description`, `show_duration`
